...
public class WinningWindowController implements Initializable {
...
t5.getKeyFrames().add(new KeyFrame(
    ...
    public void initData(Player player) {
        System.out.println("init wining controller");
    newPlayer = player;
    ...
    System.out.println("Player username at winning controller init: " + newPlayer.getUsername());
    newPlayer.setScore(player.getScore()+20);
    if((newPlayer.getLevelCode()+1)<=3) {
    //Level newLevel = new Level(newPlayer.getLevelCode()+1);
    //newPlayer.setLevel(newLevel);
    //newPlayer.setLevelCode(newLevel.getCode());
    }
    playerScoreLb.setText(String.valueOf(newPlayer.getScore()));
     ...
     Session session = HibernateUtil.getSessionFactory().openSession();
            ...
            Player p2 = null;
            //int pid=1;
            p2 = (Player)session.get(Player.class, newPlayer.getUsername());
            p2.setScore(newPlayer.getScore());
          //  p2.setLevel(newPlayer.getLevel());
            session.update(p2);
            session.getTransaction().commit();
            session.close();
            System.out.println("Player score is:"+p2.getScore());
    }
    
    public void updateLeve(Player player, int i) {
        System.out.println("Inside update level, player new level befor if");
        ...
        Session session = HibernateUtil.getSessionFactory().openSession();
         session = HibernateUtil.getSessionFactory().openSession();
         //session.beginTrnsaction();
        Level lv = null;
         if((newPlayer.getLevelCode()+1)<=3 & (i==(newPlayer.getLevelCode()))) {
    ///Level newLevel = new Level(newPlayer.getLevelCode()+1);
                lv = (Level)session.get(Level.class, i+1);
                newPlayer.setLevel(lv);
                newPlayer.setLevelCode(lv.getCode());
                System.out.println("Inside update level, player new level: " + newPlayer.getLevelCode());
    }
              //  session.getTransaction().commit();
            ...
            p2 = (Player)session.get(Player.class, newPlayer.getUsername());  
            p2.setLevelCode(newPlayer.getLevelCode());
            session.update(p2);
            transaction.commit();
            session.close();
            System.out.println("Player score is: "+p2.getScore());
