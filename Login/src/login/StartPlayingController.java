...
public class StartPlayingController extends Thread implements Initializable {
    double backX = -1000;
...
// create a node group
    ...
    public Group g = new Group();
        ...
        Timeline timeStar = new Timeline();
        Timeline timeStone = new Timeline();
...
//        rootPane.getChildren().setAll(pane4);
    ...
    */
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        //time.play();
       // magicalStart1.setLayoutX(20);
        //magicalStart1.setLayoutY(20);
        DoubleProperty r= playBack.translateXProperty();
        //magicalStar1.translateXProperty()=r;
      //  backX = playBack.getLayoutX();
        //backX = playBack.getTranslateX();
        System.out.println("in init front, back x = " + playBack.getLayoutX());
...
sequentialTransition.play();
    ...
    private void toQuestion(MouseEvent event) throws IOException {
        /*
          FXMLLoader loaderQ = new FXMLLoader();
            loaderQ.setLocation(getClass().getResource("Question.fxml"));
            AnchorPane rootQ = loaderQ.load();
            Scene scene = new Scene(rootQ);
            QuestionController controllerQ = loaderQ.getController();
            controllerQ.initData(newPlayer, stageNo);
            /*
        AnchorPane pane = FXMLLoader.load(getClass().getResource("Main menu.fxml"));
        Scene gameScene = new Scene(pane);*/
        /*
        ...
        window.showAndWait();*/
            ...
            qLoader.setLocation(getClass().getResource("Question.fxml"));
            Parent qRoot = qLoader.load();
            Scene qScene = new Scene(qRoot);
            
          //  WinningWindowController controller2 = winLoader.getController();
          //  FXMLLoader winLoader = new FXMLLoader(getClass().getResource("winningWindow.fxml"));
            //winLoader.setLocation(getClass().getResource("winningWindow.fxml"));
            QuestionController qController = qLoader.getController();
            qController.initData(newPlayer, stageNo);
//            System.out.println(winController.getClass());

                 //  winRoot = (Parent) winLoader.load();
   ...
   // newGame = game;
    ...
    newPlayer.setUsername(player.getUsername());
     ...
     public void AnimationFront(){
        ...
        translate.setDuration(Duration.millis(1600));  
        ...
        translate.setNode(playBack);  
        translate.setCycleCount(1);
        translate.play();
        
        if (Login.mute == false) { playAudioSteps(); }
         /*
         if(backX<=2500) {
         int keyv = 200;
         if(stageNo==3){
         ...
         keyv=580;
             ...
             dur=1000;
         playBack.setLayoutX(backX);
         playBack.setTranslateX(backX);
       
       System.out.println("in animation front, back x = " + playBack.getLayoutX());
        time.getKeyFrames().addAll(
                new KeyFrame(new Duration(0),new KeyValue(playBack.translateXProperty(),0)),
                new KeyFrame(new Duration(dur),new KeyValue(playBack.translateXProperty(),-keyv))
                );
               time.play();  
      ...
      public void AnimationBack(){
       ...
       System.out.println("in animation back, back x = " + playBack.getLayoutX());
        time.getKeyFrames().addAll(
                new KeyFrame(new Duration(0),new KeyValue(playBack.translateXProperty(),0)),
                new KeyFrame(new Duration(1000),new KeyValue(playBack.translateXProperty(),+200))
                );
               time.play();  
               backX += 200;
                playBack.setLayoutX(backX);
      ...
      public void AnimationBack3(){
       ...
       public void AnimationStop(){
          System.out.println("IN ANIMATION START PLAYING");
       time.stop(); 
   }
      public void animationJump(){
        ...
        //shifting the X coordinate of the centre of the circle by 400   
        translateBack.setByX(-350);  //مقدار القفز
        playBack.setTranslateX(translateBack.getByX()-250); //نقطة بداية القفز
        playBack.setLayoutX(translateBack.getByX()-250);
        backX += playBack.getTranslateX()+230;
        
        
        //setting the duration for the Translate transition   
        translateBack.setDuration(Duration.millis(1000));  
          
        //setting cycle count for the Translate transition   
        //translate.setCycleCount(500);  
          
        //the transition will set to be auto reversed by setting this to true   
        translateBack.setAutoReverse(false);  
          
        //setting Circle as the node onto which the transition will be applied  
        translateBack.setNode(playBack);  
        translateBack.setCycleCount(1);
        //playing the transition   
       // translate.play();
        //t.play();
         TranslateTransition translateBack2 = new TranslateTransition();  
          
        //shifting the X coordinate of the centre of the circle by 400   
        translateBack2.setByY(-150);  
          
        //setting the duration for the Translate transition   
        translateBack2.setDuration(Duration.millis(2000));  
          
        //setting cycle count for the Translate transition   
        //translate.setCycleCount(500);  
          
        //the transition will set to be auto reversed by setting this to true   
        translateBack2.setAutoReverse(false);  
          
        //setting Circle as the node onto which the transition will be applied  
        translateBack2.setNode(playBack);  
        translateBack2.setCycleCount(1);
        
         TranslateTransition translateBack3 = new TranslateTransition();  
          
        //shifting the X coordinate of the centre of the circle by 400   
        //translateBack3.setByX(-1000);  
        ...
        //setting Circle as the node onto which the transition will be applied  
        translateBack3.setNode(playBack);  
        translateBack2.setCycleCount(1);
        ...
        parallelTransition.getChildren().addAll(translateBack,translate);
        SequentialTransition sequentialTransition = new SequentialTransition();
        sequentialTransition.getChildren().addAll(parallelTransition, translateBack2);
        sequentialTransition.play();
//        t.play();
        //playBack.setLayoutX(backX-310);
        //playBack.setTranslateX(backX-310);
}
      
       public void setStage(int i) throws FileNotFoundException {
       stageNo = i;
        System.out.println("StageNO from start playing: " + stageNo);
        if(stageNo==1) 
            playBack.setImage( new Image(new FileInputStream("C:\\Users\\owner\\Documents\\NetBeansProjects\\Login\\src\\images\\playBackS1.png")));
        else if(stageNo==2)
            playBack.setImage( new Image(new FileInputStream("C:\\Users\\owner\\Documents\\NetBeansProjects\\Login\\src\\images\\playBackS2.png")));
        else if(stageNo==3)
            playBack.setImage( new Image(new FileInputStream("C:\\Users\\owner\\Documents\\NetBeansProjects\\Login\\src\\images\\playBackS3.png")));
         if(stageNo==1 || stageNo==2) {
    noLoopsLb.setVisible(false);
    noLoopsTf.setVisible(false);
    }
       }
       
       
     public int noOfLoops() {
   noLoops = Integer.parseInt(noLoopsTf.getText());
 TranslateTransition translate = new TranslateTransition();  
          
        //shifting the X coordinate of the centre of the circle by 400   
        translate.setByX(-200*noLoops);  
          
        //setting the duration for the Translate transition   
        translate.setDuration(Duration.millis(1600*noLoops));  
    ...
    TranslateTransition translate = new TranslateTransition();  
      ...
      void playAudioSteps(){
       AudioClip a1 = new AudioClip(this.getClass().getResource("steps.mp3").toString());
       a1.play();
    }
