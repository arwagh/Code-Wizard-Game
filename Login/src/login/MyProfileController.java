...
t5.getKeyFrames().add(new KeyFrame(
   ...
   @FXML
    ...
    playerLevel.setText(String.valueOf(newPlayer.getLevelCode()));
    playerScore.setText(String.valueOf(newPlayer.getScore()));
    
    Session session = HibernateUtil.getSessionFactory().openSession();
            session = HibernateUtil.getSessionFactory().openSession();
            session.beginTransaction();
    Level lv = (Level)session.get(Level.class, newPlayer.getLevelCode() );
    session.close();
    
    newPlayer.setLevel(lv);           
    playerNickname.setText(lv.getName());
    }
}
